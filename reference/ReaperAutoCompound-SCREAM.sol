...
//Lending Strategy
contract ReaperAutoCompoundScream is Ownable, Pausable {
    ...
    address[] public WftmToTokenRoute;
    ...
    address public treasury = 0x0e7c5313E9BB80b654734d9b7aB1FB01468deE3b;
     ...
     */
    uint256 public borrowRate = 72;
    uint256 public borrowRateMax = 75;
    uint256 public borrowDepth = 4;
    uint256 public minLeverage = 1000;
    ...
    constructor(
        ...
        scToken = _scToken;
        markets = [_scToken];
        token = ISToken(scToken).underlying();
        comptroller = ISToken(scToken).comptroller();


        WftmToTokenRoute = [wftm, token];
    ...
    function deposit() public whenNotPaused {
        uint256 tokenBal = availableWant();
    ...
    /**
     * @dev Repeatedly supplies and borrows {token} following the configured {borrowRate} and {borrowDepth}
     * @param _amount amount of {token} to leverage
     */
    function _leverage(uint256 _amount) internal {
        if (_amount < minLeverage) { return; }
        for (uint i = 0; i < borrowDepth; i++) {
            ISToken(scToken).mint(_amount);
            _amount = _amount.mul(borrowRate).div(100);
            ISToken(scToken).borrow(_amount);
        }
     ...
     * collateral. Continues to do this until it repays the entire debt and withdraws all the supplied {token}
     * from the system
     */
    function _deleverage() internal {
        uint256 tokenBal = IERC20(token).balanceOf(address(this));
        uint256 borrowBal = ISToken(scToken).borrowBalanceCurrent(address(this));

        while (tokenBal < borrowBal) {
            ISToken(scToken).repayBorrow(tokenBal);

            borrowBal = ISToken(scToken).borrowBalanceCurrent(address(this));
            uint256 targetSupply = borrowBal.mul(100).div(borrowRate);

            uint256 supplyBal = ISToken(scToken).balanceOfUnderlying(address(this));
            ISToken(scToken).redeemUnderlying(supplyBal.sub(targetSupply));
            tokenBal = IERC20(token).balanceOf(address(this));
        }

        ISToken(scToken).repayBorrow(uint256(-1));

        uint256 scTokenBal = IERC20(scToken).balanceOf(address(this));
        ISToken(scToken).redeem(scTokenBal);
     ...
     */
    function rebalance(uint256 _borrowRate, uint256 _borrowDepth) external onlyWhitelisted {
    ...
    function chargeFees() internal {
        uint256 toWftm = IERC20(scream).balanceOf(address(this));
        IUniswapRouterETH(uniRouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(toWftm, 0, ScreamToWftmRoute, address(this), now.add(600));
        uint256 wftmFees = IERC20(wftm).balanceOf(address(this)).mul(totalFee).div(PERCENT_DIVISOR);
    ...
    // swap rewards to {token}
    function swapRewards() internal {
        uint256 wftmBal = IERC20(wftm).balanceOf(address(this));
        IUniswapRouterETH(uniRouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(wftmBal, 0, WftmToTokenRoute, address(this), now.add(600));
    }
     ...
     * @param _amount How much {token} to withdraw.
     */
    function withdraw(uint256 _amount) external {
        ...
        if (tx.origin == owner() || paused()) {
            ...
            uint256 withdrawalFeeAmount = tokenBal.mul(securityFee).div(PERCENT_DIVISOR);
            IERC20(token).safeTransfer(vault, tokenBal.sub(withdrawalFeeAmount));
        }
    ...
    /**
     * @dev Required for various functions that need to deduct {reserves} from total {token}.
     * @return how much {token} the contract holds without reserves
     */
    function availableWant() public view returns (uint256) {
        uint256 tokenBal = IERC20(token).balanceOf(address(this));
        return tokenBal.sub(reserves);
    }
    ...
    // calculate the total underlaying 'token' held by the strat.
    ...
    function _giveAllowances() internal {
        IERC20(token).safeApprove(scToken, uint256(-1));
        IERC20(wftm).safeApprove(uniRouter, uint256(-1));
        IERC20(scream).safeApprove(uniRouter, uint256(-1));
    }

    function _removeAllowances() internal {
        IERC20(token).safeApprove(scToken, 0);
        IERC20(scream).safeApprove(uniRouter, 0);
    }
    /**
