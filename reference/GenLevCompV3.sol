...
library FlashMintLib {
    ...
    ComptrollerI private constant COMPTROLLER =
    ...
    function doFlashMint(
        ...
        (uint256 requiredDAI, uint256 amountWant) = getFlashLoanParams(
            want,
            amountDesired,
            collatRatioDAI
        );
    ...
    function getFlashLoanParams(
        ...
        (priceDAIWant, decimalsDifference, requiredDAI) = getPriceDAIWant(
            want,
            amountDesired,
            collatRatioDAI
        );
        ...
        )
    {
        if (want == DAI) {
            ...
            priceDAIWant = getOraclePrice(DAI).mul(PRICE_DECIMALS).div(
                getOraclePrice(want)
            );
    ...
    function getOraclePrice(address token) internal returns (uint256) {
        ...
        IUniswapAnchoredView oracle = IUniswapAnchoredView(
            COMPTROLLER.oracle()
        );
...
contract Strategy is BaseStrategy, IERC3156FlashBorrower {
    ...
    function setFourThreeProtection(bool _fourThreeProtection)
        external
        management
    {
    ...
    function getblocksUntilLiquidation() public view returns (uint256) {
        ...
        uint256 collateralisedDeposit1 = deposits
    ...
    function predictCompAccrued() public view returns (uint256) {
        ...
        uint256 distributionPerBlockSupply = compound.compSupplySpeeds(
            address(cToken)
        );
        uint256 distributionPerBlockBorrow = compound.compBorrowSpeeds(
            address(cToken)
        );
    ...
    function doFlashMint(bool deficit, uint256 amountDesired)
        internal
        returns (uint256)
    {
        return
